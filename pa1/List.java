...
//If 0<=i<=length()-1, moves the cursor to the element at index i, otherwise the cursor becomes undefined.
     void moveTo(int i){
         ...
         int j = 0;
         if(i<0){
              throw new RuntimeException("List Error in moveTo(): inputed index is not positive.");
         }if(i>(this.length)-1){
              throw new RuntimeException("List Error in moveTo(): inputed index is beyond lenght of list.");
...
//Deletes cursor element in this List. Curosr is undefined after this operation. Pre: length()>0, getIndex()>=0
           void delete(){
...
//Returns a new list representing the same integer sequence as this list. The cursor in the new list is 
//undefined, regardless of the state of the cursor in this List. This List is unchanged.
           List copy(){
